...
code = 688208 #这里输入需要画k线的股票代码

def get_data():

    sql = f'''
    #一个个股过去100天的数据
    select ds.secu_code, ds.secu_abbr, a.end_date, a.open, a.lowest, a.highest, a.close,a.volume
    from dw_quote a
            join dim_secumain ds on a.secu_id = ds.secu_id
    where ds.secu_code = {code}
    # and DATEDIFF(a.end_date, NOW()) <= 0
    # AND DATEDIFF(a.end_date, NOW()) > -100
    order by a.end_date
    ;
    '''
    data1 = Connect.query_lugutong_pro(sql)
    return split_data(data=data1)

def split_data(data):
    category_data =data['end_date'].dt.strftime("%Y-%m-%d").tolist()
    data2 = data[['end_date','open','close','highest','lowest','volume']]
...
def draw_charts():
    ...
    bar = (
        ...
        .set_global_opts(
            title_opts=opts.TitleOpts(title="k线图", subtitle={code}),
